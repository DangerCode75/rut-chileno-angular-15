...
export class RutComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (!this.msjError) this.msjE = 'El rut ingresado no es válido.';
    else this.msjE = this.msjError;
  }
  ...
  validaRUT_(rut: string): boolean {
    ...
    if (dvEsperado.toString() !== dv && cuerpo.length >= 0) return true;
    else return false;
  }
  ...
  sendEmiterRut(rut: string): void {
    if (!this.validacionRut) {
      switch (this.mode) {
        ...
        case 3:
          let r: string = rut;
          this.rut_emiter.emit(
            this.rutClean(r).slice(0, -1) + '-' + r.slice(-1).toUpperCase()
          );
          break;
        case 4:
          let ru: string = rut;
