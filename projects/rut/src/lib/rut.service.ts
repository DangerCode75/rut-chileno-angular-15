...
export class RutService {
  ...
  rutFormat(value: string): string | undefined {
    const rut: string | any = this.rutClean(value);
    if (rut.length <= 1) {
      return;
    }
  ...
  rutClean(value: string): string | undefined {
  ...
  getRutChile(mode: number, rut: string): string | boolean | undefined {
    if (!this.validaRUT(rut)) {
      switch (mode) {
        ...
        case 3:
          let r: string | any = rut;
          r = this.rutClean(r);
          const c = r.slice(0, -1);
          let dv = r.slice(-1).toUpperCase();
          return c + '-' + dv;
        case 4:
          let ru: string | any = rut;
          ru = this.rutClean(ru);
  ...
  getRutChileForm(mode: number, rut: string): string | undefined {
    switch (mode) {
